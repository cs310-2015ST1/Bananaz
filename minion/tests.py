...
from garden.models import FoodTree
from garden.views import search_criteria, global_var_garden
...
def create_http_request_object(name, list_of_foods):
    ...
    qdict.update(dict)
    object.GET = qdict
    assert (object.GET.has_key('name'))
    assert (object.GET.has_key('foods'))
    return object
...
class SearchCriteriaTests(TestCase):
    def test_search_for_a_garden_with_name_success(self):
        # request = RequestFactory().get('search_criteria/', data={
        #     'name': 'One',
        #     'foods': []
        # })
        ...
        self.assertEqual(global_var_garden.pop(0).name == 'One', True )
